...
* @return {Array} tree
*/
var setFileObjectReferences = function(tree, files, pathProperty, childrenProperty){
    eachTreeNode(tree, function(node, parent, depth){
        var path = node.path;
        if(files[path]){
            if(pathProperty){
            ...
            if(childrenProperty){
                node.file[childrenProperty] = node.children;
            }
        }
        node.parent = parent;
    });
...
var getNav = function(navName, config, files){
    ...
    // ignored if mergeMatchingFilesAndDirs == true
    else if(!includeDirs){
        nodes = removeDirs(nodes);
    }

    nodes = setFileObjectReferences(nodes, files, pathProperty, childrenProperty);
...
var plugin = function plugin(navConfigs, settings){
    ...
    return function(files, metalsmith, done){
        ...
        //         var p = '';
        //         if(node.file){
        //             p = node.file.nav_path;
        //         }
        //         console.log(tab + '- ' + node.path, p);
        ...
        //     console.log(key, files.nav_path);
        // }
