...
* @method filterFiles
* @param {Object} files - Metalsmith files object
* @param {Function} filter - filter function accepting fileName, fileObject params
...
/**
* @method filterFilesByPropValueMatch
* @param {Object} files - Metalsmith files object
* @param {String} propKey - file metadata key
* @param {String|Array} val - file metadata key value to check for
* @return {Object} filtered files object
*/
var filterFilesByPropValueMatch = function(files, propKey, val){
    return filterFiles(files, function(key, file){
        var groups = file[propKey];

        if(typeof groups === 'string' && groups === val){
            return true;
...
var getNav = function(navName, config, navFiles, settings){
    ...
    if(filterProperty){
        navFiles = filterFilesByPropValueMatch(navFiles, filterProperty, filterValue);
    }

    if(filter){
        navFiles = filterFiles(navFiles, filter);
    }
